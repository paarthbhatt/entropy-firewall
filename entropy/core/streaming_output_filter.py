"""Streaming Output Filter — sliding window PII detection for streaming responses.
    ...
    async def process_stream(
        ...
        if self.buffer:
            ...
            self.total_detections.extend(
                [{"rule": r.name, "category": r.category} for r in self.rules if r.pattern.search(self.buffer)]
            )
            yield final
