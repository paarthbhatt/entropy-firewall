"""Model Downloader ΓÇö fetch ONNX classifier model on first use.
...
def ensure_model(
    ...
    # No URL configured ΓÇö cannot download
    if not url:
        logger.info(
...
def download_model_cli() -> None:
    ...
    from entropy.config import get_settings  # noqa: PLC0415
    ...
    if not url:
        ...
        print("Set ENTROPY_SEMANTIC_MODEL_URL or configure in config.yaml")
    ...
    if result:
        print(f"Γ£à Model ready at: {result}")
    else:
        print("Γ¥î Model download failed. Check logs for details.")
