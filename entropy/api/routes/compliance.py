...
@router.get(
    ...
    description="Returns blocked and critical requests from the audit log, formatted as compliance violations.",
)
    ...
    for idx, row in enumerate(rows):
        ...
        violations.append(
            ViolationRecord(
                ...
                threats=[t.get("type", t) if isinstance(t, dict) else str(t) for t in (threats_raw or [])],
...
async def get_compliance_stats(
    ...
    weighted_violations = sum(
        threat_counts.get(t, 0) * w for t, w in weights.items()
    )
...
async def submit_override(
    ...
    if body.action == "FALSE_POSITIVE":
        ...
        if feedback_store:
            try:
                from entropy.services.learning.feedback_store import FeedbackRecord
...
async def generate_guardrails(
    request: Request,
    file: UploadFile = File(..., description="Policy PDF (max 50MB)"),
    ...
    try:
        from entropy.providers.registry import get_registry
