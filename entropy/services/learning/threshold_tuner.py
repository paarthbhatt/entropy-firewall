...
from entropy.services.learning.feedback_store import FeedbackStore, PatternStats
...
class ThresholdTuner:
    ...
    1. High false positive rate (>30%) → decrease threshold sensitivity
    2. High accuracy (>95%) with many samples → increase sensitivity
        ...
        for stats in all_stats:
            ...
            if fp_rate > self.HIGH_FP_RATE:
                adjustment = await self._adjust_threshold(
                    ...
                    direction="decrease",
                    reason=f"High FP rate: {fp_rate:.1%} ({stats.incorrect_count}/{stats.total_feedback})",
                )
    ...
    async def get_adjusted_threshold(self, pattern_name: str) -> Optional[float]:
...
__all__ = ["ThresholdTuner", "ThresholdAdjustment"]